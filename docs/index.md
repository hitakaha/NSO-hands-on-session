...
!!! info "Two-day flow"
    Labs **1–9** run on the standard NSO **{{ nso_version }}** sandbox (Day 1). Labs **10–11** are a **Day 2** continuation using the dCloud **"NSO 6.7 MCP"** scenario — a NSO release, and the new **Model Context Protocol** server. Each chapter declares its own NSO version in front-matter.
